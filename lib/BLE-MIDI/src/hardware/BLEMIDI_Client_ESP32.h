...
#define BLEMIDI_CLIENT_COMM_MAX_INTERVAL 80  // 40ms
...
/** Set the security method.
     ...
     *  More info in nimBLE lib
     */
#define BLEMIDI_CLIENT_SECURITY_AUTH                       \
  (BLEMIDI_CLIENT_BOND_DUMMY | BLEMIDI_CLIENT_MITM_DUMMY | \
   BLEMIDI_CLIENT_PAIR_DUMMY)
  ...
  void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
    if (enableConnection)  //not begin() or end()
    {
      ...
      if (advertisedDevice->isAdvertisingService(NimBLEUUID(SERVICE_UUID))) {
        ...
        if (!specificTarget ||
            (advertisedDevice->getName() == nameTarget.c_str() ||
             advertisedDevice->getAddress() == nameTarget)) {
...
class BLEMIDI_Client_ESP32 {
 private:
  ...
  bool firstTimeSend =
      true;  //First writeValue get sends like Write with reponse for clean security flags. After first time, all messages are send like WriteNoResponse for increase transmision speed.
  ...
  friend class MIDI_NAMESPACE::MidiInterface<
      BLEMIDI_Transport<BLEMIDI_Client_ESP32>, MySettings>;  //
 ...
 public:
  ...
  void write(uint8_t* data, uint8_t length) {
    ...
    if (!_characteristic->writeValue(data, length,
                                     !_characteristic->canWriteNoResponse()))
      firstTimeSend = true;
 ...
 protected:
  ...
  void connectCallbacks(
      MIDI_NAMESPACE::MidiInterface<BLEMIDI_Transport<BLEMIDI_Client_ESP32>,
                                    MySettings>* MIDIcallback);
...
class MyClientCallbacks : public BLEClientCallbacks {
 ...
 protected:
  ...
  uint32_t onPassKeyRequest() { return userOnPassKeyRequest(); };
  ...
  bool onConnParamsUpdateRequest(NimBLEClient* pClient,
    ...
    if (params->itvl_min <
        BLEMIDI_CLIENT_COMM_MIN_INTERVAL) { /** 1.25ms units */
      return false;
    } else if (params->itvl_max >
               BLEMIDI_CLIENT_COMM_MAX_INTERVAL) { /** 1.25ms units */
      return false;
    } else if (
        params->latency >
        BLEMIDI_CLIENT_COMM_LATENCY) { /** Number of intervals allowed to skip */
      return false;
    } else if (params->supervision_timeout >
               BLEMIDI_CLIENT_COMM_TIMEOUT) { /** 10ms units */
      return false;
    }
...
bool BLEMIDI_Client_ESP32::begin(
    const char* deviceName,
    BLEMIDI_Transport<class BLEMIDI_Client_ESP32>* bleMidiTransport) {
...
#else
    strDeviceName =
        BLEMIDI_CLIENT_NAME_PREFIX + strDeviceName + BLEMIDI_CLIENT_NAME_SUBFIX;
#endif
  }
  ...
  mRxQueue =
      xQueueCreate(256, sizeof(uint8_t));  // TODO Settings::MaxBufferSize

  NimBLEDevice::setSecurityIOCap(
      BLEMIDI_CLIENT_SECURITY_CAP);  // Attention, it may need a passkey
...
bool BLEMIDI_Client_ESP32::available(byte* pvBuffer) {
  if (myAdvCB.enableConnection) {
    if (_client == nullptr ||
        !_client->isConnected())  //Try to connect/reconnect
    {
    ...
    return xQueueReceive(mRxQueue, (void*)pvBuffer,
                         0);  // return immediately when the queue is empty
  } else {
    return false;
  }
}
...
void BLEMIDI_Client_ESP32::notifyCB(
...
bool BLEMIDI_Client_ESP32::connect() {
  using namespace std::
      placeholders;  //<- for bind funtion in callback notification
  ...
  if (_client) {
    if (_client ==
        NimBLEDevice::getClientByPeerAddress(myAdvCB.advDevice.getAddress())) {
      if (_client->connect(&myAdvCB.advDevice, false)) {
        if (_characteristic->canNotify()) {
          if (_characteristic->subscribe(
                  true, std::bind(&BLEMIDI_Client_ESP32::notifyCB, this, _1, _2,
                                  _3, _4))) {
  ...
  _client->setConnectionParams(
      BLEMIDI_CLIENT_COMM_MIN_INTERVAL, BLEMIDI_CLIENT_COMM_MAX_INTERVAL,
      BLEMIDI_CLIENT_COMM_LATENCY, BLEMIDI_CLIENT_COMM_TIMEOUT);
