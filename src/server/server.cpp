...
void PipoServer::setup() {
  ...
  // name should depend on the type of pipo, but for now config page has no mean
...
void PipoServer::setup_requests() {
  ...
  server.on("/wifi-status", HTTP_GET, [&](AsyncWebServerRequest *request) {
    return request->send(200, "text/plain", wm.status().c_str());
  });
  ...
  server.on(
      ...
      [&](AsyncWebServerRequest *request, String filename, size_t index,
        ...
        // Here I am doing save first then load. so parsing happen with load
        // function. this avoids parsing in here and trying to pass the json to
        // config.set(). after solving other issues, not sure if this has any
        // value after all.
