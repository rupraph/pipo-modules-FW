...
void AnalogSensor::update() {
  ...
  for (auto const& pair : touch_map) {
    ...
    // float val =
    // lp_filter_map[pair.first].process(touchRead(pair.second))-sensor_dat[pair.first].offset;
    ...
    // Todo: should find a way that changing the sensor limit also propagates to
    // the connected midi translator
    // }
    // else
