...
// Todo engine.
// should create a table for axis config. min max, etc, since this is shared for
// both hid and midi should push axis enabling in here instead of in the sensor
// class will have to add error catching: -> when config could not load for eg.
// could use combination mode to have note from orientation, and trigger from
// acceleration find way to map scales/arpegio over axis (for analog when touch
// is a note) should use callback to trigger events and notes find way to:
  ...
  for (auto const &pair : sensor_dat) {
    ...
    if (sensor.get_enabled(axis_name) &&
        ...
        Miditranslators[axis_name].enabled ==
            true) // Todo: temporary. should likely have "in_use" to trigger
                  // note on/off
    {
      // if CC MODE
        ...
        if (Miditranslators[axis_name].getHires()) {
          ...
          uint8_t cc_val =
              max(0, min(Miditranslators[axis_name].get_cc_val(
                         ...
                         127));
          // for midi find way to limit rotation to max 180° to avoid overflow
          // to 0
...
// probaly get triggered should be something linked to the deadzone
        ...
        // is different from previous note
        ...
        if (sensor.get_triggered(axis_name) &&
          ...
          // should probably move the value check in the io class. to be
          // discussed
        ...
        if (sensor_val < sensor.get_limit_max(axis_name) &&
            !midiio.is_note_playing(
                note_val[channel],
                channel)) // midiio.channel_note_list[channel].find(note_val[channel])
                          // == midiio.channel_note_list[channel].end())
        {
          midiio.sendNoteOn(note_val[channel], 127, channel, 800);
        }
