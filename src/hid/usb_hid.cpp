...
void usb_hid::usb_hid_setup()
{
    ...
    {
    case 0:
        usb_hid_port.setReportDescriptor(gamepad_hid_report, sizeof(gamepad_hid_report));
        break;
    case 1:
        usb_hid_port.setReportDescriptor(mouse_hid_report, sizeof(mouse_hid_report));
        break;
    case 2:
...
{   
    ...
    //keyboard should not have a continous report or this will likley cause redundant keypresses
    // case 2:
...
{ // Todo: unsure if I shoudl stupidly send keys then release as long as they are "pressed"
    if ( !usb_hid_port.ready() ) return;
