...
class Pwm {
 ...
 public:
  ...
  typedef struct mem {
    ...
    double
        ke;  // servo easing constant (0.0 linear, >0 and <1 sigmoid, 1.0 no easing)
  ...
  mem_t mem[8] = {  // channel data (see above)
  ...
  uint8_t attachInvert(
      int pin);  // attach pin to next free channel with inverted pwm
  uint8_t attachInvert(int pin,
                       int ch);  // attach to specified ch with inverted pwm
  ...
  uint8_t attachServo(int pin, int ch, int minUs, int maxUs, double speed,
                      double ke);
  uint8_t attachServo(int pin, int ch, int minUs, int maxUs, double speed,
                      double ke, bool invert);
  float read(int pin);
  ...
  // tone and note
  void tone(int pin, uint32_t frequency, uint16_t duration = 500,
            uint16_t interval = 0);
  void note(int pin, note_t note, uint8_t octave, uint16_t duration,
            uint16_t interval);
 ...
 private:
  ...
  void config_servo(int ch, int minUs, int maxUs, double speed = 0,
                    double ke = 1.0);
