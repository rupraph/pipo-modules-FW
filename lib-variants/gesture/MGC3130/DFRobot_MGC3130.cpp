...
bool DFRobot_MGC3130::begin(void)
{
  _pWire->begin();
    ...
    // ESP32/ESP8266 may need explicit I2C clock frequency setting
    _pWire->setClock(100000);  // Set I2C clock to 100kHz (standard speed)
  #endif
  tsInput();
  reset();
...
uint8_t DFRobot_MGC3130::read(void* pBuf, size_t size)
{
  ...
  uint8_t * _pBuf = (uint8_t *)pBuf;
  if(tsRead() != 0){
   DBG("tsPin HIGH, no data to read");
   return 0;
  }
  tsOutput();
  tsWrite(LOW);
  ...
  // Note: requestFrom() already completes the I2C transaction, 
  // no need to call endTransmission() here
  tsWrite(HIGH);
  tsInput();
  delay(5);
  return size;
